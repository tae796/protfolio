...
<head>
	...
	function updatefunc(id) {
		window.location.href="/Promotion/Promotion_update?id="+id;
	}
...
<section id="side-wrap">
 ...
 <header id="header">
    ...
    <section id="best">
        <div class="cont">
        <c:forEach var="popular" items="${list}" begin="0" end="2"> 
            <div class="box">
            <div class="img" >
    <img src="/resources/img/${popular.image}" style="height:100%">
    </div>
            <h2><c:out value="${popular.name}"/></h2>
            <p><fmt:formatNumber value="${popular.price}" pattern="#,###" /><span> 원</span></p>
             </div>
               </c:forEach>
        </div>
    </section>
    ...
    <section id="car">
        <div class="cont">
            <div class="title">
             ...
             <c:forEach var="popular" items="${list}" begin="3" end="8"> 
    ...
    height: 100%;
">
						<img src="../../resources/img/${popular.image}"
						style=" height: 100%; background-size: contain; width: 100%;">
                </div>
            </div>
...
">
                <div class="top">
                    <h1><c:out value="${popular.name}"/></h1>
                </div>
                <div class="middle">
                    <h2>기간 <div></div> <span><c:out value="${popular.start_ddt}"/> ~ <c:out value="${popular.end_ddt}"/></span></h2>
                    <table>
                        <tr>
                            <td>상품</td><td>
							 <c:choose>
...
</td><td>등급</td><td><c:out value="${popular.grade}"/></td><td>연료</td><td><c:out value="${popular.oil_type}"/></td>
                        </tr>
                        <tr>
                            <td>배기량</td><td><c:out value="${popular.cc}"/>cc</td><td>연비</td><td><c:out value="${popular.oil_rate}"/>km/l</td><td>약정거리</td><td><c:out value="${popular.agreement_dist}"/>km/년</td>
                        </tr>
                        <tr>
                            <td>담보조건</td><td><c:out value="${popular.guarantee}"/></td>
                        </tr>
                        <tr>
                            <td>옵션</td><td><c:out value="${popular.option}"/></td>
                        </tr>
                    </table>
                </div>
                <div class="bottom">
                    <div class="money">
                        <h2><c:out value="${popular.months}"/>개월</h2>
                        <span><fmt:formatNumber value="${popular.price}" pattern="#,###" />원</span>
                    </div>
                    <button><a href="/Request/Ask?type=${popular.product_type}#target">
                        <h2>고객맞춤</h2>
                        <h2>상담신청</h2>
                    </a></button>
                      <c:choose>
            	...
            	<button onclick="updatefunc(${popular.id});" class="write">수정</button>
            	<button onclick="deletefunc(${popular.id});" class="write">삭제</button>
            </div>
            </c:when>
		<c:otherwise>
		</c:otherwise>
